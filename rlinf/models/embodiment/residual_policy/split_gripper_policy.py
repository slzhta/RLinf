...
"""Six Gaussian arm residuals and an independently BC-initialized gripper."""
...
class SplitGripperPolicy(ResidualPolicy):
    """Joint PPO distribution N(arm_mean, arm_std) times Bernoulli(gripper)."""
        ...
        if "logprobs" in result or "entropy" in result:
            ...
            if "logprobs" in result:
                ...
                result["logprobs"] = torch.cat(
                    (
                        result["logprobs"][..., :6],
                        distribution.log_prob((action > 0).to(distribution.logits.dtype)),
                    ),
                    -1,
                )
