...
class ResidualPolicy(CNNPolicy):
    """Reuse CNNPolicy's encoder, tanh-Gaussian PPO policy and value head.
    ...
    together with the unscaled tanh-squashed action using the normal CNN rollout schema.
    """
    ...
    def __init__(self, cfg: ResidualConfig) -> None:
        ...
        if cfg.binary_action_indices or cfg.action_scale is not None:
            raise ValueError(
                "Residual PPO requires the native continuous tanh-Gaussian policy."
            )
    ...
    def default_forward(
        ...
        """Evaluate the native tanh-Gaussian on the saved active action channels."""
